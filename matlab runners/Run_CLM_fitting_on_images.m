function [ err_outline, err_no_outline ] = Run_CLM_fitting_on_images(output_loc, varargin)
...
if(any(strcmp(varargin, 'use_afw')))
    afw_loc = [getenv('USERPROFILE') '/Dropbox/AAM/test data/AFW/'];
    dataset_dirs = cat(1, dataset_dirs, afw_loc); 
end
if(any(strcmp(varargin, 'use_lfpw')))
    lfpw_loc = [getenv('USERPROFILE') '/Dropbox/AAM/test data/lfpw/testset/'];
    dataset_dirs = cat(1, dataset_dirs, lfpw_loc); 
end
if(any(strcmp(varargin, 'use_ibug')))
    ibug_loc = [getenv('USERPROFILE') '/Dropbox/AAM/test data/ibug/'];
    dataset_dirs = cat(1, dataset_dirs, ibug_loc); 
end
if(any(strcmp(varargin, 'use_helen')))
    helen_loc = [getenv('USERPROFILE') '/Dropbox/AAM/test data/helen/testset/'];
...
%%
dirs = {[getenv('USERPROFILE') '/Dropbox/AAM/test data/AFW/'];
    [getenv('USERPROFILE') '/Dropbox/AAM//test data/ibug/'];
    [getenv('USERPROFILE') '/Dropbox/AAM//test data/helen/testset/'];
    [getenv('USERPROFILE') '/Dropbox/AAM//test data/lfpw/testset/'];};
