...
% clamp x and y so as not to be out of bounds
if( x(1) < 1 || x(1) > size(mymat, 2))
    %fprintf('warning:x1 (%f) is not within range for line drawing', x);
    x(1) = min(max(x(1), 1), size(mymat,2));
end
if( y(1) < 1 || y(1) > size(mymat, 1))
    %fprintf('warning:y1 (%f) is not within range for line drawing', y);    
    y(1) = min(max(y(1), 1), size(mymat,1));
end
if( x(2) < 1 || x(2) > size(mymat, 2))
    %fprintf('warning:x1 (%f) is not within range for line drawing', x);
    x(2) = min(max(x(2), 1), size(mymat,2));
end
if( y(2) < 1 || y(2) > size(mymat, 1))
    %fprintf('warning:y1 (%f) is not within range for line drawing', y);    
    y(2) = min(max(y(2), 1), size(mymat,1));
end
